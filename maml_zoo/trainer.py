...
class Trainerintr(object):
    ...
    def __init__(
        ...
        self.intr_optimizer = tf.train.AdamOptimizer()

        self.regressor = self.intr_optimizer.minimize(loss=self.predictor.regress_loss, var_list=self.predictor.policy_params)
    ...
    def train(self):
        ...
        with self.sess.as_default() as sess:
            ...
            for itr in range(self.start_itr, self.n_itr):
                ...
                logger.log("Processing samples...")
                time_proc_samples_start = time.time()
                samples_data = self.sample_processor.process_samples(paths, log='all', log_prefix='train-',compute_intr=True)
                proc_samples_time = time.time() - time_proc_samples_start
                ...
                #hidden_state = self.sess.run(self.predictor._zero_hidden)
                #self.predictor.reset([True]*200)
                self.sess.run([self.regressor],feed_dict={self.predictor.input_var: samples_data['observations'][:128,...],
                                                                self.predictor.true_rew: samples_data['ori_rewards'][:128,...],
                                                          self.predictor.hidden_var:self.predictor._hidden_state})
                #self.intr_optimizer.optimize(input_val_dict={self.predictor.input_var: samples_data['observations'],
                #                                                self.predictor.true_rew: samples_data['ori_rewards']})
                """ ------------------- Logging Stuff --------------------------"""
                logger.logkv('Itr', itr)
                logger.logkv('n_timesteps', self.sampler.total_timesteps_sampled)
                logger.logkv('Number of env steps total', self.sampler.total_timesteps_sampled)
