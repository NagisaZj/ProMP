...
from maml_zoo.trainer import Trainer, Trainerintr
from maml_zoo.samplers.maml_sampler import MAMLSampler
from maml_zoo.samplers.rl2_sample_processor import RL2SampleProcessor, ERL2SampleProcessor
from maml_zoo.policies.meta_gaussian_mlp_policy import MetaGaussianMLPPolicy
from maml_zoo.policies.gaussian_rnn_policy import GaussianRNNPolicy
from maml_zoo.policies.predictor import  RNN
import os
...
def main(config):
    ...
    predictor = RNN(input_dim=obs_dim,output_dim=1,cell_type='lstm')
    sampler = MAMLSampler(
        env=env,
        policy=policy,
    ...
    sample_processor = ERL2SampleProcessor(
        baseline=baseline,
        discount=config['discount'],
        gae_lambda=config['gae_lambda'],
        normalize_adv=config['normalize_adv'],
        positive_adv=config['positive_adv'],
        exp_episode_length=config['exp_episode_length'],
        predictor=predictor,
        intr_rew_weight=0.1
    )
...
if __name__=="__main__":
    ...
    data_path =  './data/erl2-intr/cheetah-vel/test_%d' % idx
    logger.configure(dir=data_path, format_strs=['stdout', 'log', 'csv'],
                     snapshot_mode='last_gap')
    config = json.load(open(maml_zoo_path + "/configs/erl2_intr_config_cheetah_vel.json", 'r'))
