...
from rlkit.core.rl_algorithm import MetaRLAlgorithm,ExpAlgorithmIter,ExpAlgorithmFin, ExpAlgorithmFin2
...
class ExpSACFin(ExpAlgorithmFin):
    ...
    def sample_sac(self, indices):
        ...
        if self.use_per:
            ...
            batches = [ptu.np_to_pytorch_batch(self.replay_buffer.random_batch(idx, batch_size=self.batch_size)) for
                       idx in indices]
        unpacked = [self.unpack_batch(batch, sparse_reward=False) for batch in batches]
        # group like elements together
        unpacked = [[x[i] for x in unpacked] for i in range(len(unpacked[0]))]
        unpacked = [torch.cat(x, dim=0) for x in unpacked]
        return unpacked
    ...
    def _take_step(self, indices,context_pred):
        ...
        actions = actions.view(t * b, -1)
        rewards = rewards.view(t * b, -1)
        rewards = rewards * self.prediction_loss_scale
    ...
    def _take_step_exp(self, indices,context_unbatched,context_pred,context):
        ...
        obs, actions, agent_rew, next_obs, terms = context
        obs = obs.contiguous()
        t,b,_ = context_pred.size()
        ...
        if self.intrinsic_reward_weight>0:
            if self.use_kl_div_intrinsic:
                ...
                intrinsic_reward = (pred_rew - agent_rew) ** 2
                if self.consider_dynamics:
...
class ExpSACFin2(ExpAlgorithmFin2):
    def __init__(
        ...
        self.qf1, self.qf2, self.vf = nets[1:]
        self.qf1_exp, self.qf2_exp, self.vf_exp, self.rew_decoder, self.transition_decoder, self.pie_snail = nets_exp[1:]
        self.target_exp_vf = self.vf_exp.copy()
        self.target_vf = self.vf.copy()
        ...
        self.pie_snail_optimizer = optimizer_class(
            self.pie_snail.parameters(),
            lr=context_lr,
        )
    ...
    @property
    def networks(self):
        return  [self.context_encoder] + [self.exploration_agent.policy] + [self.qf1_exp, self.qf2_exp, self.vf_exp, self.target_exp_vf,self.rew_decoder,self.transition_decoder] + [self.pie_snail,self.agent.policy,self.qf1, self.qf2, self.vf, self.target_vf]
    ...
    def sample_context(self, indices,sequence=True):
        ''' sample batch of context from a list of tasks from the replay buffer '''
        # make method work given a single task index
        if not hasattr(indices, '__iter__'):
            indices = [indices]
    ...
    def _do_training(self, indices):
        ...
        for i in range(num_updates):
            ...
            self._take_step_exp(indices, exp_context_unbatched,exp_context_pred)

            # stop backprop


    def _min_q_exp(self,  actions,z):
        #print(obs.shape,actions.shape)

        q1 = self.qf1_exp(torch.cat([ actions,z],dim=1))
        q2 = self.qf2_exp(torch.cat([ actions,z],dim=1))
        min_q = torch.min(q1, q2)
        return min_q
    ...
    def _take_step(self, indices, context_unbatched,context_pred):
        t,b,_ = context_pred.size()
        num_tasks = len(indices)
        z_s = self.context_encoder.forward_seq(context_pred)
        z_s = z_s.view(t,b,-1)
    ...
    def _take_step_exp(self, indices,context_unbatched,context_pred):

        self.pie_snail_optimizer.zero_grad()
        obs, actions, _, next_obs, terms, info, agent_rew = context_unbatched
        obs = obs.contiguous()
        t,b,_ = context_pred.size()
        z_s = self.context_encoder.forward_seq(context_pred)
        ...
        z_mean_prev, z_var_prev, z_mean_post, z_var_post = z_mean_prev.contiguous(), z_var_prev.contiguous(), z_mean_post.contiguous(), z_var_post.contiguous()
        z_mean_prev, z_var_prev,z_mean_post,z_var_post = z_mean_prev.view(t*(b-1),-1), z_var_prev.view(t*(b-1),-1),z_mean_post.view(t*(b-1),-1),z_var_post.view(t*(b-1),-1)
        pie_z = self.pie_snail.forward_seq(context_pred)
        pie_z = pie_z.view(t,b,-1)
        pie_z_pre, pie_z_post = pie_z[:,:-1,:], pie_z[:,1:,:]
        pie_z_pre, pie_z_post = pie_z_pre.contiguous(), pie_z_post.contiguous()
        pie_z_pre = pie_z_pre.view(t*(b-1),-1)
        pie_z_post = pie_z_post.view(t * (b - 1), -1)
        policy_outputs,_ = self.exploration_agent( pie_z_pre)
        ...
        # Q and V networks
        # encoder will only get gradients from Q nets
        q1_pred = self.qf1_exp(torch.cat([actions,pie_z_pre],dim=1))
        q2_pred = self.qf2_exp(torch.cat([actions,pie_z_pre],dim=1))
        v_pred = self.vf_exp(torch.cat([pie_z_pre],dim=1))
        # get targets for use in V and Q updates

        with torch.no_grad():
            target_v_values = self.target_exp_vf(torch.cat([pie_z_post],dim=1))   #pre
        ...
        # compute min Q on the new actions
        new_actions = new_actions.view(t * b, -1)
        min_q_new_actions = self._min_q_exp(new_actions,pie_z_pre.detach())
        ...
        if self.eval_statistics_2 is None:
            ...
            self.eval_statistics_2['QF Loss Exp'] = np.mean(ptu.get_numpy(qf_loss))
            self.eval_statistics_2['VF Loss Exp'] = np.mean(ptu.get_numpy(vf_loss))
    ...
    def get_epoch_snapshot(self, epoch):
        ...
        snapshot = OrderedDict(
            context_encoder=self.context_encoder.state_dict(),
            pie_snail=self.pie_snail.state_dict(),
            qf1_exp=self.qf1_exp.state_dict(),
            qf2_exp=self.qf2_exp.state_dict(),
            policy_exp=self.exploration_agent.state_dict(),
            vf_exp=self.vf_exp.state_dict(),
