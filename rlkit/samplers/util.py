...
def exprolloutsimple(env, agent, max_path_length,  max_trajs, accum_context_for_agent=False, context_agent = None,reward_bias=1e-3,rsample=1):
    """
    The following value for the following keys will be a 2D array, with the
    first dimension corresponding to the time dimension.
     - observations
    ...
    return dict(
        ...
        agent_infos=agent_infos,
        env_infos=env_infos,
        values=np.array(sparse_rewards).reshape(-1, 1) + reward_bias
    )

def exprollout_splitsimple(env, agent, max_path_length,  max_trajs, accum_context_for_agent=False, context_agent = None,reward_bias=1e-3,rsample=1):
    """
    The following value for the following keys will be a 2D array, with the
    first dimension corresponding to the time dimension.
     - observations
