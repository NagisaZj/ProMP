...
class ExpInPlacePathSamplerSimple(object):
    ...
    ```
    """
    def __init__(self, env, policy, max_path_length,encoder,reward_bias=1e-3):
        self.env = env
        self.policy = policy
    ...
    def obtain_samples(self, deterministic=False, max_samples=np.inf, max_trajs=np.inf, accum_context_for_agent=False, resample=1, context_agent = None,split=False,rsample=1):
        ...
        if not split:
            ...
            self.env, policy, max_path_length=self.max_path_length, max_trajs=max_trajs, accum_context_for_agent=accum_context_for_agent, context_agent = context_agent,reward_bias=self.reward_bias,rsample=rsample)
            paths.append(path)
            n_steps_total += len(path['observations'])
            n_trajs += 1
            ...
            path = exprollout_splitsimple(
                ...
                accum_context_for_agent=accum_context_for_agent, context_agent=context_agent,reward_bias=self.reward_bias,rsample=rsample)
